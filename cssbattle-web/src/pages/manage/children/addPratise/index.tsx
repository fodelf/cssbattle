...
const Choice: React.FC<any> = (props: IProps) => {
  ...
  const queryPratiseDetail = () => {
    (formRef.current as any).setFieldsValue({
      ...
      choiceContent:
        (props.formObj?.options &&
          props.formObj?.options.map((item: any) => item.des)) ||
        [],
      answer:
        props.formObj?.type === 1
          ? props.formObj?.answer
          : props.formObj?.answer[0],
    });
  };
  ...
  return (
    ...
    >
      ...
      <Form.Item
        label="答案"
        name="answer"
        rules={[{ required: true, message: '请选择答案选项' }]}
      >
        {props.type === '0' ? (
          <Radio.Group>
          ...
          <Checkbox.Group>
            {choiceList.map((item, index) => {
              return (
                <Checkbox value={String.fromCharCode(65 + index)}>
                  {String.fromCharCode(65 + index)}
                </Checkbox>
              );
            })}
